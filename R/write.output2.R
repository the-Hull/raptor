#7.write.output----
write.output2<-function(input,location=c("./"),flip=FALSE){
      ...
      for(u in c(1:length(years)) ){
            ...
            rows<-max(unique(data_year[,"ROW"])[which(!is.na(unique(data_year[,"ROW"])))])
      ...
      output_all_years <- do.call(rbind, outlist)
