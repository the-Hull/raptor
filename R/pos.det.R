...
#' @param list a \code{\link{data.frame}} of the year of the ring and "CID" (unique cell ID) from cells which should be considered as first row cells. Should be filled if \code{\link{first.cell}} output is not used as input. The \code{\link{data.frame}} should be ordered to years (first column) and present specific "CID" (second column) which is considered the first cell.
...
pos.det<-function(input,swe = 0.5,sle = 3,ec = 1.75,swl = 0.25,sll = 5,lc = 5,prof.co =6,
  ...
  for(u in c(1:length(yrs))){
    ...
    if(length(which(colnames(data_year)=="ROW"))==1){
      ...
      if(is.numeric(list_y[,2])==FALSE)stop('list is not numeric')
      data_year[,"ROW"]<-NA
      for(a in c(1:length(list_y[,2]))){
        data_year[which(data_year[,"CID"]==list_y[a,2]),"ROW"]<-a
      }
    }
    
    which(is.na(data_year$ROW)==F)
