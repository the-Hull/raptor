...
#'
#' @description Returns a \code{\link{data.frame}}, i.e. the same as produced by \code{\link{first.cell}}, but with added column showing the ROW and POSITION which indicate the radial files number (ROW, from left to right) and radial cell position (from earlywood to latewood) of each tracheid. \code{\link{NA}} is assigned to tracheids not belonging to recognized radial files. A MARKER column is added to indicate the last detected cell in the earlywood search (indicated with 1), latewood search (2), the last detected cell (3), rows that are removed due to gaps (4, see prof.co) and rows that are removed due to limited amount of cells (5, see max.cells).
#' @param input an input file as produced by \code{\link{first.cell}}.
#' @param swe a numeric values that is multiplied by the square rooted cell lumen area (l) of the target cell and used to determine the width of the rectangle search area which locates the next earlywood cell in the row (default = 0.5).
#' @param sle a numeric that multiplied by the square rooted cell lumen area (l) of the target cell to determine the length of the rectangle search area which locates the next earlywood cell in the row (default = 3).
...
#' @param prof.co threshold ratio between the distance to the previous and consecutive cell to determine if the row (or radial file) has to be excluded (default = 6).
...
#' @param list a \code{\link{data.frame}} of "CID" from cells which should be considered as first row cells. Should be filled if \code{\link{first.cell}} output is not used as input. The vector should be ordered to years and present specific "CID" which is considered the first cell.
