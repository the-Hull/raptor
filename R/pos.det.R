#' Detect position of tracheids in rows and radial files.
#'
...
#'
#' @param input	An input data frame as produced by first.cell().
#' @param swe	Numeric value that is multiplied by the square-rooted cell lumen
...
#' @param sle	a numeric value that is multiplied by the square rooted cell lumen
...
#' @param ec	threshold ratio between the lumen area of two consecutive earlywood
...
#'   cell.
#' @param swl	a numeric value that is multiplied by the square rooted cell lumen
...
#' @param sll	a numeric value that is multiplied by the square rooted cell lumen
#'   area (l) of the target cell to determine the length of the rectangle search
#'   area which locates the next earlywood cell in the row (default = 5).
#' @param lc	threshold ratio between the lumen area of two consecutive latewood
...
#' @param prof.co	threshold ratio between the distance to the previous and
#'   consecutive cell to determine if the row (or radial file) should be
#'   excluded (default = 6).
#' @param max.cells	threshold proportion of the maximum number of cells to
#'   determine if the radial file has to be excluded (default = 0.6).
#' @param aligning	logical. Indicates whether a second alignment has to be
#'   performed based upon the cells detected within first.cell() (default =
#'   TRUE).
#' @param list	a data frame of CID (unique cell ID) from cells which should be
...
#' @param yrs	either a vector providing the year(s) of interest or FALSE to
#'   select all years included in input (Default = FALSE).
#' @param make.plot	logical. Indicating whether to make a plot (default =
#'   FALSE).

#' @return Returns a original data frame with added columns for tracheid row and
