...
#'
#' @description Provides a re-aligned \code{\link{is.raptor}} file, where new XCAL and YCAL values are calculated according to a specified rotation. Appropriate alignment is achieved when the first row of earlywood cells are positioned along an approximately horizontal axis and the radial files of tracheids are positioned along a vertical axis.
#' @param input an \code{\link{is.raptor}} file.
#' @param year a numerical value specifying the year of interest. The default (\code{\link{FALSE}}) applies the function on all years present in the input.
